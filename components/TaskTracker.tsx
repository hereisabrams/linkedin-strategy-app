...
export const TaskTracker: React.FC<TaskTrackerProps> = ({ user }) => {
  ...
  return (
    <div className="bg-slate-800 rounded-lg shadow-lg p-6 sticky top-8">
      <h2 className="text-2xl font-bold text-white mb-4">Daily Growth Tasks</h2>
      <div className="space-y-6">
        <div>
          <div className="flex justify-between items-baseline mb-2">
            <h3 className="font-semibold text-white">Follow New Connections</h3>
          ...
          <div className="w-full bg-slate-700 rounded-full h-2.5">
            <div
              className={`h-2.5 rounded-full transition-all duration-500 ${isComplete ? 'bg-green-500' : 'bg-brand-blue'}`}
          ...
          <div className="flex gap-2 mt-4">
             <button
              ...
              disabled={isComplete}
              className="w-full bg-brand-blue text-white font-semibold py-2 px-4 rounded-md hover:bg-blue-500 transition-colors disabled:bg-slate-600 disabled:cursor-not-allowed"
            >
        ...
        <div className="border-t border-slate-700 pt-4">
            <h3 className="font-semibold text-white">Engage with 5 Posts</h3>
            <p className="text-sm text-gray-400 mt-1">Leave a thoughtful comment on 5 posts from people in your target audience. (Manual Tracking)</p>
        </div>
      </div>
    </div>
  );
